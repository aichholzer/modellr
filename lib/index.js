...
class Modellr extends EventEmitter {
  ...
  close(connection = null) {
    if (!connection) {
      ...
      this.models = [];
    } else if (this.modellrSequelizeInstances[connection] && !this.modellrSequelizeInstances[connection].unloaded) {
  ...
  instance(alias = null) {
    ...
    if (alias && alias in this.modellrSequelizeInstances && !this.modellrSequelizeInstances[alias].unloaded) {
...
module.exports = new Proxy(modellr, {
  get: (target, name) => {
    if ([
      ...
      '_eventsCount',
      '_maxListeners'
    ].includes(name) || name.toString().startsWith('modellr')) { return target[name]; }
